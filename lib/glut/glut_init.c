...
WNDPROC glut_winproc = NULL;

LONG WINAPI
default_winproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	LONG result;

	if (glut_winproc != NULL) {
		result = (*glut_winproc)(hwnd,msg,wParam,lParam);
		if (result==0) return result;
	}

	result = __glutWindowProc(hwnd,msg,wParam,lParam);
	return result;
}

void APIENTRY glutWinProc(WNDPROC* w) {
	glut_winproc = w;
}
...
__glutOpenWin32Connection(char* display)
{
  ...
  wc.style         = CS_OWNDC;
  wc.lpfnWndProc   = (WNDPROC)default_winproc;
