...
int APIENTRY glutMatchHWND(HWND hwnd) {
	int i;
	for (i=0; i<__glutWindowListSize; i++) {
		if (!__glutWindowList[i]) continue;
		if (__glutWindowList[i]->win == hwnd) {
			return i;
		}
	}
	return -1;
...
HWND APIENTRY glutGetHWND(int w) {
	if (w >=0 && w < __glutWindowListSize) {
		return __glutWindowList[w]->win;
	}
	else {
		return -1;
	}
